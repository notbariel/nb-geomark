...
class CountryShortcodeService
{
    public function getCountryByShortcode($shortcode)
    {
        return $this->countries()->has($shortcode) ? $this->countries()->get($shortcode)[0] : null;
    }

    public function getRegionByShortcode($countryShortcode, $regionShortcode)
    {
        if (
            !$this->countries()->has($countryShortcode) ||
            !isset($this->countries()->get($countryShortcode)[2]) ||
            count($this->countries()->get($countryShortcode)[2]) <= 0
        ) {
            return null;
        }

        $regions = collect($this->countries()->get($countryShortcode)[2]);

        $region = $regions->first(function ($region) use ($regionShortcode) {
            return $region[1] === $regionShortcode;
        });
    ...
    {
        // taken from https://github.com/country-regions/country-region-data
        return collect([
            "AF" => [
                "Afghanistan",
                "AF",
